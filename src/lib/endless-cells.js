    ...
    return {
                ...
                style: {
                    ...
                    items: {
                        ...
                        colgroup: {
                            type: "string",
                            ref: "colgroup",
                            label: "Col. width (%; comma sep.; max 100)",
                            expression: ""
                        },
                        fixed: {
                            type: "boolean",
        ...
        paint: function ($element, layout) {
            ...
            if (layout.cells) {
                ...
                try {
                    columnsWidth = layout.colgroup.split(',');
                } catch (ex) {

                }
                if (columnsWidth.length == (maxColumn + 1)) {
                    colGroup = '<colgroup>';
                    for (var c = 0; c < maxColumn + 1; c++) {
                        colGroup += '<col style="width:' + columnsWidth[c].replace('%', '') + '%">';
                    }
                    colGroup += '</colgroup>';
                }

                if(layout.fixed == true) {
                    layout.tablecss += ';table-layout:fixed;'
                }

                if(layout.wrap == true) {
                    layout.tablecss += ';word-break: break-all;'
                }                

                html = '<table id="myFinalTable" style="' + layout.tablecss + '">' + colGroup + html + '</table>';
